...
uint8_t ReceiveID_Setting[4]={0x00,0x00,0x00,0x00};
 ...
 if(G_OBD_Control_Status==false)
{
 ...
 if((data[0]&0x0F) ==0x3)
 {
	G_OBD_Receive_CMD =true;
	 ...
	 BLE_Receive_Service_index = data[1]&0x0F;
	 BLE_Receive_Service_ID_List[BLE_Receive_Service_index]= ((uint32_t)data[2]<<24)+((uint32_t)data[3]<<16)+((uint32_t)data[4]<<8)+data[5];
	 ReceiveID_Setting[0]=data[2];
	 ReceiveID_Setting[1]=data[3];
	 ReceiveID_Setting[2]=data[4];
	 ReceiveID_Setting[3]=data[5];
	 	
	 if(debugmode)
	{
		PRINTF("\r\n Service_ID + %d \n",BLE_Receive_Service_ID_List[BLE_Receive_Service_index]);
	}
	if(BLE_Receive_Service_ID_List[BLE_Receive_Service_index]<0x800)
	{
			 CAN_SetRxIndividualMask(CAN0, BLE_Receive_Service_index, CAN_RX_MB_STD(BLE_Receive_Service_ID_List[BLE_Receive_Service_index], 0));
	}
	else
	{
